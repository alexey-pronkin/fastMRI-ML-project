...
class UnetGenerator3D(nn.Module):
    def __init__(self, d=32, mean=0, std=0.02):
        ...
        self.conv6 = nn.Conv3d(d * 8, d * 8, 4, 2, 0)
        ...
        self.deconv3 = nn.ConvTranspose3d(d * 8 * 2, d * 8, 4, 2, 0)
    ...
    def forward(self, input):
        ...
        e3 = self.conv3_bn(self.conv3(F.leaky_relu(e2, 0.2)))
        e4 = self.conv4_bn(self.conv4(F.leaky_relu(e3, 0.2)))
        e5 = self.conv5_bn(self.conv5(F.leaky_relu(e4, 0.25)))
        e6 = self.conv6_bn(self.conv6(F.leaky_relu(e5, 0.25)))
        e7 = self.conv7_bn(self.conv7(F.leaky_relu(e6, 0.25)))
        e8 = self.conv8(F.leaky_relu(e7, 0.2))
...
class Discriminator3D(nn.Module):
    ...
    def forward(self, input, label):
        ...
        x = F.leaky_relu(self.conv1(x), 0.2)
        x = F.leaky_relu(self.conv2_bn(self.conv2(x)), 0.2)
        x = F.leaky_relu(self.conv3_bn(self.conv3(x)), 0.25)
        x = F.leaky_relu(self.conv4_bn(self.conv4(x)), 0.2)
        x = torch.sigmoid(self.conv5(x))
